...
Original file is located at
    https://colab.research.google.com/drive/1-Hypi4pyRvK1IDyWBmWQp_TNBR1DM0Lt
...
def compute_grads(args, dense_out_lin, pruned_out_lin, H, weight_indexer, device='cpu'):
  ...
  dW_dense = dense_out_lin.weight.grad

  pruned_out_lin.zero_grad()
  dense_out_lin.zero_grad()

  return dW_pruned, dW_dense, loss.item()

"""### step 2: find best s columns of grad outside S and merge with S"""

@torch.no_grad()
def find_and_merge(args, dW, S, n_heads_og, dim_per_head, n_heads_to_keep, device='cpu'):
  dW_by_head = dW.view(dW.shape[0], n_heads_og, dim_per_head).to(device)
  importance = torch.norm(dW_by_head, p=args.norm_order, dim=(0, 2))
  # importance = torch.sum(dW_by_head, dim=(0, 2))

  for index in S:
    importance[index] = 0
  imp_top_idxs = torch.argsort(torch.abs(importance), descending=True)[:n_heads_to_keep]
  imp_top_idxs = set(imp_top_idxs.tolist())

  D = imp_top_idxs.difference(S)
  D_indexer = get_weight_indexer(D, dim_per_head)

  # print(f'importances of grad: {importance}')
...
@torch.no_grad()
def update_step(args, pruned_out_lin, dense_out_lin, dW_pruned, dW_dense, D_indexer, S_indexer, H, device=None):
  Q = torch.zeros_like(dense_out_lin.weight.data)

  eta = args.eta
  if args.eta == 'adaptive':
    eta = torch.norm(dW_dense) ** 2 / torch.norm(H @ dW_dense.T) ** 2

  Q[:, D_indexer] = dense_out_lin.weight.data[:, D_indexer] + eta * dW_dense[:, D_indexer]
  if S_indexer is not None:
    Q[:, S_indexer] = pruned_out_lin.weight.data - eta * dW_pruned
...
@torch.no_grad()
def truncate(args, Q, pruned_out_lin, h, n_heads_og, dim_per_head, n_heads_to_keep, dW, device='cpu'):
  if args.trunc_strategy == 'magnitude':
    Q_by_head = Q.view(Q.shape[0], n_heads_og, dim_per_head).to(device)
    imp = torch.norm(Q_by_head, p=args.norm_order, dim=(0, 2))
    # imp = torch.sum(Q_by_head, dim=(0, 2))
    ...
    imp = torch.norm(dW_by_head, p=args.norm_order, dim=(0, 2))
    # imp = torch.sum(dW_by_head, dim=(0, 2))

  else:
    raise NotImplementedError()

  imp_top_idxs = torch.argsort(torch.abs(imp), descending=True)[:n_heads_to_keep]
  S = set(imp_top_idxs.cpu().tolist())
  not_S = set(range(n_heads_og)).difference(S)
...
def debias(args, pruned_out_lin, dense_out_lin, H, S_indexer, iters, device='cpu'):
  ...
  for i in range(iters):
    ...
    with torch.no_grad():
      U_dense = dense_out_lin(H)
      U_pruned = H_pruned @ pruned_out_lin.weight.data.T #+ pruned_out_lin.bias.data
    U_pruned = pruned_out_lin(H_pruned)
    ...
    with torch.no_grad():
      dW_pruned = pruned_out_lin.weight.grad
      dBias_pruned = pruned_out_lin.bias.grad
      ...
      pruned_out_lin.bias.data = pruned_out_lin.bias.data - eta * dBias_pruned

  return pruned_out_lin
...
def residual_objective(mat):
    return 0.5 * torch.norm(mat, p='fro') # using torch.sum requires small stepsize (order of 1e-5)
...
def preprocess_function(examples, tokenizer):
    ...
    for i, offset in enumerate(offset_mapping):
        ...
        if len(answer["answer_start"]) < 1 or len(answer["text"]) < 1:
          start_char = 0
          end_char = 0
        else:
...
def validate(args, model, ids, inputs, answers, metric, tokenizer, device='cpu'):
  ...
  for i in range(num_examples):
    ...
    sep_token_index = inputs.input_ids[i].tolist().index(tokenizer.sep_token_id)
    no_answer_prob = torch.softmax(outputs_sp.start_logits[i][sep_token_index], dim=-1).item()

    pred = {'id': ids[i], 'prediction_text': pred_sp, 'no_answer_probability': no_answer_prob}
    preds.append(pred)
    ...
    if prune_type in ['random', 'ispasp', 'ispasp++']:
        for i, rat in enumerate(ratios):
            ...
            if prune_type == 'ispasp++':
                ...
                layers[i].attention = prune_attn_layer_no_Q(args, layers[i].attention, data_list, mask_list, val_arguments, prune_ratio=rat, layer_id=i, device=device)
...
def prune_attn_layer_no_Q(args, layer, hidden_states_list, mask_list, val_arguments, prune_ratio=0.5, layer_id=None, device='cpu'):
  if layer_id is not None:
    print(f'\nPruning layer {layer_id}')

  layer.pruned_heads = set([])
  n_heads_to_keep = int(layer.self.num_attention_heads * prune_ratio)
  n_heads = layer.self.num_attention_heads
  attn_head_size = layer.self.attention_head_size
  ...
  pruned_out_lin = nn.Linear(in_features = layer.self.attention_head_size * n_heads_to_keep,
                             out_features = dense_out_lin.bias.shape[0],
                             bias = True).to(device)
  ...
  with torch.no_grad():
      ...
      h = torch.norm(shape(H), p=args.norm_order, dim=(0, 2, 3))
  ...
  for t in range(args.iters):
    ...
    dW_pruned, dW_dense, loss = compute_grads(args, dense_out_lin, pruned_out_lin, H, S_indexer, device=device)
    D, D_indexer = find_and_merge(args, dW_dense, S, n_heads, attn_head_size, n_heads_to_keep, device=device)

    if args.validate_iter:
      print(f'Iteration {t}: S = {S} | Loss = {loss} | Exact Match = {acc["exact_match"]} | F1 = {acc["f1"]}')
    elif args.iter_verbose:
      print(f'Iteration {t}: S = {S}| D = {D} | Loss = {loss}')

    Q = update_step(args, pruned_out_lin, dense_out_lin, dW_pruned, dW_dense, D_indexer, S_indexer, H, device=device)
    pruned_out_lin, S, S_indexer = truncate(args, Q, pruned_out_lin, h, n_heads, attn_head_size, n_heads_to_keep, dW_dense, device=device)
    pruned_out_lin = debias(args, pruned_out_lin, dense_out_lin, H, S_indexer, args.debias_iters, device=device)
...
args = {
    ...
    'tokenizer_name': 'deepset/bert-base-cased-squad2', #'distilbert-base-uncased-distilled-squad',
    'model_name': 'deepset/bert-base-cased-squad2',
    'dataset_name': 'squad_v2',

    'random_seed': 0,

    'train_size': 1000, # INCREASE TO SIZE OF TRAINING DATASET
    'val_size': 100, # INCREASE
    ...
    'debias_iters': 10,
    'trunc_strategy': 'magnitude', # magnitude dense_grad attn_weights
    'choose_from_D': False
}
d_args = dotdict(args)
...
results_dict = {'metrics': run_mets, 'args': args}
